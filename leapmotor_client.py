...
def build_parser() -> argparse.ArgumentParser:
    ...
    remote_ctl_dry_run_parser = subparsers.add_parser(
        "direct-login-remote-ctl-dry-run",
        help="Login with static app cert and build a remote/ctl request without sending it.",
    )
    ...
    remote_ctl_dry_run_parser.add_argument(
        "--action",
        required=True,
        help="Human action label or candidate command value, e.g. lock/unlock/trunk/window/find_car.",
    )
    remote_ctl_dry_run_parser.add_argument(
        "--payload",
        help="Exact form body to sign/show. If omitted, safe candidate bodies are generated.",
    )
    remote_ctl_dry_run_parser.add_argument(
        "--content-type",
        default="application/x-www-form-urlencoded",
        choices=["application/x-www-form-urlencoded", "application/json"],
        help="Content-Type for the dry-run request.",
    )
    remote_ctl_dry_run_parser.add_argument(
        "--sign-mode",
        default="status_like",
        choices=["status_like", "legacy", "picture_like", "userid_false", "userid_true"],
        help="Remote-control sign hypothesis to use.",
    )
    remote_ctl_dry_run_parser.add_argument(
        "--account-p12-password",
        default=None,
    ...
    unlock_dry_run_parser = subparsers.add_parser(
        "direct-login-unlock-dry-run",
        help="Login with static app cert and build the verified unlock flow without sending vehicle-control requests.",
    )
    ...
    unlock_dry_run_parser.add_argument(
        "--account-p12-password",
        default=None,
        help="Optional fallback password for the account PKCS12; normally derived from login data.",
    )

    unlock_execute_parser = subparsers.add_parser(
        "direct-login-unlock",
        help="Execute the verified unlock flow. This sends live vehicle-control requests and requires --execute.",
    )
    ...
    unlock_execute_parser.add_argument(
        "--account-p12-password",
        default=None,
        help="Optional fallback password for the account PKCS12; normally derived from login data.",
    )
    unlock_execute_parser.add_argument(
        "--execute",
        action="store_true",
        help="Actually send operPwd/verify and remote/ctl. Omit this flag to keep the command blocked.",
    )

    lock_dry_run_parser = subparsers.add_parser(
        "direct-login-lock-dry-run",
        help="Login with static app cert and build the verified lock flow without sending vehicle-control requests.",
    )
    ...
    lock_dry_run_parser.add_argument(
        "--account-p12-password",
        default=None,
        help="Optional fallback password for the account PKCS12; normally derived from login data.",
    )

    lock_execute_parser = subparsers.add_parser(
        "direct-login-lock",
        help="Execute the verified lock flow. This sends live vehicle-control requests and requires --execute.",
    )
    ...
    lock_execute_parser.add_argument(
        "--account-p12-password",
        default=None,
        help="Optional fallback password for the account PKCS12; normally derived from login data.",
    )
    ...
    climate_dry_run_parser = subparsers.add_parser(
        "direct-login-climate-dry-run",
        help="Login with static app cert and build a verified climate-control flow without sending vehicle-control requests.",
    )
    ...
    climate_dry_run_parser.add_argument(
        "--action",
        required=True,
        choices=sorted(CLIMATE_ACTION_SPECS.keys()),
        help="Verified climate action to build.",
    )
    climate_dry_run_parser.add_argument(
        "--account-p12-password",
        default=None,
    ...
    climate_execute_parser = subparsers.add_parser(
        "direct-login-climate",
        help="Execute a verified climate-control flow. This sends live vehicle-control requests and requires --execute.",
    )
    ...
    climate_execute_parser.add_argument(
        "--action",
        required=True,
        choices=sorted(CLIMATE_ACTION_SPECS.keys()),
        help="Verified climate action to execute.",
    )
    climate_execute_parser.add_argument(
        "--account-p12-password",
        default=None,
    ...
    remote_ctl_read_probe_parser = subparsers.add_parser(
        "direct-login-remote-ctl-read-probe",
        help="Login with static app cert and probe remote/ctl read-style sub-endpoints only.",
    )
    ...
    remote_ctl_read_probe_parser.add_argument(
        "--endpoint",
        required=True,
        choices=["getAppointment", "result/query"],
        help="Read-style remote/ctl sub-endpoint to probe.",
    )
    remote_ctl_read_probe_parser.add_argument(
        "--action",
        help="Optional action hint to include in some probe candidates, e.g. lock/unlock.",
    )
    remote_ctl_read_probe_parser.add_argument(
        "--operation-password",
        help="Optional operation password to test only against read-style remote/ctl sub-endpoints.",
    )
    remote_ctl_read_probe_parser.add_argument(
        "--content-type",
        default="application/x-www-form-urlencoded",
        choices=["application/x-www-form-urlencoded", "application/json"],
        help="Content-Type for the probe request.",
    )
    remote_ctl_read_probe_parser.add_argument(
        "--sign-mode",
        default="status_like",
    ...
    operpwd_verify_probe_parser = subparsers.add_parser(
        "direct-login-operpwd-verify-probe",
        help="Login with static app cert and probe vehicle/v1/operPwd/verify only.",
    )
    ...
    operpwd_verify_probe_parser.add_argument(
        "--content-type",
        default="application/x-www-form-urlencoded",
        choices=["application/x-www-form-urlencoded", "application/json"],
        help="Content-Type for the verify request.",
    )
    operpwd_verify_probe_parser.add_argument(
        "--sign-mode",
        default="status_like",
        choices=["status_like", "legacy", "picture_like", "userid_false", "userid_true"],
        help="Sign hypothesis to use.",
    )
    operpwd_verify_probe_parser.add_argument(
        "--account-p12-password",
        default=None,
    ...
    operpwd_context_probe_parser = subparsers.add_parser(
        "direct-login-operpwd-context-probe",
        help="Login once, then safely compare operPwd/verify across cert modes and bootstrap sequences.",
    )
    ...
    operpwd_context_probe_parser.add_argument(
        "--cert-mode",
        default="both",
        choices=["app", "account", "both"],
        help="Which client certificate context to test for operPwd/verify.",
    )
    operpwd_context_probe_parser.add_argument(
        "--bootstrap-sequence",
        action="append",
        choices=[
            ...
            "device-save",
            "sync-timeOffset",
            "device-save+sync-timeOffset",
            "cert-sync",
        ],
        help="Optional safe pre-verify sequence. Can be repeated; default tests only 'none'.",
    )
    operpwd_context_probe_parser.add_argument(
        "--account-p12-password",
        default=None,
